...
class PADE_Platform_Linkedin {
	...
	public static function send( array $payload ): array {
		...
		$token    = (string) ( $settings['platform_api_tokens']['linkedin'] ?? '' );
		...
		if ( empty( $token ) ) {
			$result = array(
				'success'   => false,
				'http_code' => 0,
				'response'  => array( 'message' => 'linkedin token missing.' ),
			);
			PADE_Logger::log( 'linkedin', $post_id, 0, $result['response'], 0 );
			return $result;
		}
		...
		$http_code = 200;
		$response  = array(
			'message' => 'linkedin simulated send complete',
			'body'    => $message,
		);

		PADE_Logger::log( 'linkedin', $post_id, $http_code, $response, 1 );
